...
public class DiGraph<T extends Comparable<T>> {
	 ...
	 *            Value for Vertex 2
	 * @param cost
	 *            Cost or weight of edge
	 */
	public void add(T from, T to, int cost, String direction) {
		...
		if (temp != null) {
			...
			Edge e = new Edge(from, to, cost, direction);
			edges.add(e);
		}
	}
	...
	/**
	 * Find edge containg two vertices in direction v1 -> v2
	 * 
	 * @param v1
	 *            Vertex 'from'
	 * @param v2
	 ...
	 * @return Edge, or <code>null</code> if not found.
	 */
	private Edge findEdgeByName(String v1, String v2) {
		for (Edge each : edges) {
			if (each.from.name.equals(v1) && each.to.name.equals(v2)) {
				return each;
			}
		}
		return null;
	...
	/**
	 * Performs a recursive Depth First Search on the 'root' node (the first vertex
	 * created)
	 * 
	...
	/**
	 * Performs a Breadth-First Search starting at 'root' node (First created
	 * vertex)
	 * 
	 ...
	 * 
	 * @param v1
	 *            Value of type T for starting vertex
	...
	/**
	 * Creates path information on the graph using the Dijkstra Algorithm, Puts the
	 * information into the Vertices, based on given starting vertex.
	 * 
	 * @param v1
	 *            Value of type T for starting vertex
	...
	/**
	 * Goes through the result tree created by the Dijkstra method and steps
	 * backward
	 * 
	 * @param from
	 * @param target
	 ...
	 */
	private Result getShortestPath(T from, Vertex target, Orientation orientation) {
		...
		for (Vertex v = target; v != null; v = v.previous) {
			String temp = String.valueOf(v.value);
			if (v.previous != null) {
				verticesInPath.add(v.previous.name);
			}

			Edge resultEdge = getResultEdge(verticesInPath, v, v.previous, from, target, path);

			if (resultEdge != null) {
				path.add(v.value + " : cost : " + resultEdge.cost + " direction: " + resultEdge.direction);

				// Forming Result Object
				instructions.add(new Instruction(resultEdge.direction, resultEdge.cost, resultEdge.direction, v.name));
			}
			// System.out.println("" + v.value + " : cost : " + v.minDistance + " direction:
			// " + getDirection(verticesInPath, v, v.previous, from, target, path));
		}

		Collections.reverse(instructions);

		result.setInstructions(processInstructions(instructions, orientation));
	...
	private List<Instruction> processInstructions(List<Instruction> instructions, Orientation orientation) {

		for (int i = 0; i < instructions.size(); i++) {
			if (i == 0) {
				Instruction instruction = instructions.get(0);
				String translatedDirection = translateOrientation(instruction, orientation.getDirection());
				instruction.setAction(translatedDirection);
			} else {
				Instruction instruction = instructions.get(i);
				String translatedDirection = translateOrientation(instruction,
						instructions.get(i - 1).getCurrentDirection());
				instruction.setAction(translatedDirection);
			}
		}
		return instructions;
	...
	private String translateOrientation(Instruction instruction, String currentDirection) {

		if (currentDirection.equals("N")) {
			switch (instruction.getCurrentDirection()) {
			case "N":
				return "F";
			case "E":
				return "R";
	...
	private Edge getResultEdge(List<String> verticesInPath, final DiGraph<T>.Vertex previous, DiGraph<T>.Vertex current,
			T from, DiGraph<T>.Vertex target, List<String> path) {

		if (current != null && current.name != target.name && previous.name != from) {
			return findEdgeByName(current.name, previous.name);
		}
		/*
		 * else if(path.size()>2 && previous.name!=target.name) { return findEdgeByName(
		 * previous.name, target.name).direction; }
		 */
		return null;
	...
	/**
	 * PUBLIC WRAPPER FOR PRIVATE FUNCTIONS Calls the Dijkstra method to build the
	 * path tree for the given starting vertex, then calls getShortestPath method to
	 * return a list containg all the steps in the shortest path to the destination
	 * vertex.
	 * 
	 * @param from
	 *            value of type T for Vertex 'from'
	 * @param to
	 *            value of type T for vertex 'to'
	 * @return ArrayList of type String of the steps in the shortest path.
	 */
	public Result getPath(T from, T to, Orientation orientation) {
		boolean test = Dijkstra(from);
		if (test == false)
			return null;
		Result result = getShortestPath(from, findVertex(to), orientation);
		return result;
	}
	...
	class Vertex implements Comparable<Vertex> {
		...
		/**
		 * Get string of Vertex with all it's ingoing and outgoing adjacencies @ return
		 * string
		 */
		@Override
		public String toString() {
	...
	class Edge {
		...
		/**
		 * @param v1
		 *            value of type T for 'from' vertex
		 * @param v2
		 *            value of type T for 'to' vertex
		 * @param cost
		 *            integer value for cost/weight of edge
		 */
		public Edge(T v1, T v2, int cost, String direction) {
			from = findVertex(v1);
			if (from == null) {
				from = new Vertex(v1, v1.toString());
				vertices.add(from);
			}
			to = findVertex(v2);
			if (to == null) {
				to = new Vertex(v2, v2.toString());
				vertices.add(to);
			}
			this.cost = cost;
		...
		@Override
		public String toString() {
			return "Edge From: " + from.value + " to: " + to.value + " cost: " + cost + " direction: " + direction;
		}
	}
